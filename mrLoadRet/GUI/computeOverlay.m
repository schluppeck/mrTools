    ...
    switch(overlayCType)
      ...
      case 'setRangeToMax'
        ...
        if ~isempty(overlays.overlayIm(overlayMasks(:,:,:,iOverlay)))
          overlays.colorRange(iOverlay,1) = max(overlays.colorRange(1),min(overlays.overlayIm(overlayMasks(:,:,:,iOverlay))));
          overlays.colorRange(iOverlay,2) = min(max(overlays.overlayIm(overlayMasks(:,:,:,iOverlay))),overlays.colorRange(2));
        end
